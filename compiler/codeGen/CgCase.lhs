...
cgEvalAlts cc_slot bndr alt_type@(PrimAlt tycon) alts =
  ...
  where
    ...
    cgEvalPrimAlts (VecArg {}) [(_,[],_,rhs)]
      = do  { abs_c <- forkProc $ do
                    ...
                    ; restoreCurrentCostCentre cc_slot True
                      ...
                      -- alternative
                    ;  vectorHeapCheck [] 0 nptrs noStmts (cgExpr rhs) }
    ...
    cgEvalPrimAlts rep alts
      = do  { let   reg = dataReturnConvPrim rep    -- Bottom for voidRep
