...
StgWeak *dead_weak_ptr_list, *dead_weak_ptr_list_tail;
...
initWeakForGC(void)
{
    ...
    for (g = 0; g <= N; g++) {
        ...
        gen->weak_ptr_list_tail = NULL;
    }

    weak_stage = WeakPtrs;
    dead_weak_ptr_list = NULL;
    dead_weak_ptr_list_tail = NULL;
...
{
    StgWeak *w;
    for (w = gen->old_weak_ptr_list; w; w = w->link) {
        evacuate(&w->finalizer);
        // NB. we must retain the order of the dead_weak_ptr_list (#7160)
        if (dead_weak_ptr_list == NULL)
            dead_weak_ptr_list = w;
        else
            dead_weak_ptr_list_tail->link = w;
        dead_weak_ptr_list_tail = w;
    }
}
        ...
        case WEAK:
            ...
            if (new != NULL) {
                ...
                debugTrace(DEBUG_weak,
                           "weak pointer still alive at %p -> %p, info[value]=%p",
                           w, w->key, w->value->header.info);
                ...
                *last_w = w->link;
                ...
                if (new_gen->weak_ptr_list == NULL) {
                    new_gen->weak_ptr_list = w;
                } else {
                    new_gen->weak_ptr_list_tail->link = w;
                }
                new_gen->weak_ptr_list_tail = w;
                w->link = NULL;
                flag = rtsTrue;
...
*/
        ...
        default:
            barf("traverseWeakPtrList: not WEAK: %d, %p", info->type, w);
        }
    }
...
markWeakPtrList ( void )
{
    ...
    for (g = 0; g <= N; g++) {
        ...
        for (w = gen->weak_ptr_list; w; w = w->link) {
